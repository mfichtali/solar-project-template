...
import org.solar.system.mdm.service.config.properties.KafkaProperties;
...
public class KafkaConsumerConfig {
	...
	Map<String, Object> consumerConfigs() {
		...
		props.put(ConsumerConfig.GROUP_ID_CONFIG, MDM_GROUP_ID);
	...
	RecordFilterStrategy<String, KafkaDomain> applyFilter() {
		return record -> {
			...
			return isMdmGroupMatch(groupsHeaderAsString);
		};
	}

	private static boolean isMdmGroupMatch(String groupsHeaderAsString) {
		boolean groupMatch = true;
		if(StringUtils.isNotBlank(groupsHeaderAsString)) {

			if(groupsHeaderAsString.contains(",")) {
				groupsHeaderAsString = KafkaGroupHeaderEnum.filterOfMdm(groupsHeaderAsString.split(","));
			}

			if (groupsHeaderAsString != null) {
				groupMatch = KafkaGroupHeaderEnum.groupMdm().stream().noneMatch(groupsHeaderAsString::equals);
			}
		}
		return groupMatch;
	}
